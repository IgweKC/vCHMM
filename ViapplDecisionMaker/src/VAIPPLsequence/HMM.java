...
/**
     ...
     * about the sequence of states</p>. <p> The idea of HMM can be extended to a
     * scenario where the state is partially visible or visible but without clear meaning </p>
     *
     * @author KC Igwe
     */
    public class HMM<O> implements vSingleSequenceLabeler<O> {
        ...
        /**
         * Generates  possible observations from 3 obseavtion sets
         ...
         *
         * @param o an observation sequence.
         * @param stt a state sequence corresponding to the observations.
         * @return the most likely state sequence.
         */
        public int predict(int[] o, int[] stt) {
            // The probability of the path.
            double[] trellis = new double[o.length];
            // probable next state.
            int nextState = 0;
            // forward
            trellis[0] = log(pi[stt[0]]) + log(b[stt[0]][o[0]]);
            for (int i = 1; i < o.length; i++) {
                trellis[i] = trellis[i-1] + log(a[stt[i-1]][stt[i]]) + log(b[stt[i]][o[i]]);
            }
            int n = o.length - 1;

            double maxDelta = Double.NEGATIVE_INFINITY;
            for (int i = 0; i < numStates; i++){
                double nextTrellis = trellis[n] + log(a[stt[n]][i]);
                if(nextTrellis > maxDelta){
                    maxDelta =  nextTrellis;
                    nextState = i;
                }
            }
         ...
         * @param labels the state labels of observations, of which states take
         * values in [0, p), where p is the number of hidden states.
         *  @param allPossibleObservations the possible observations including the ones yet to be observed
         */
        public HMM(O[][] observations, int[][] labels, O[] allPossibleObservations) {
            if (observations.length != labels.length) {
                throw new IllegalArgumentException("The number of observation sequences and that of label sequences are different.");
            }
            ...
            pi = new double[numStates];
            a = new double[numStates][numStates];
            b = new double[numStates][allPossibleObservations.length];
         ...
         * @return a new hidden markov model with the same parameter
         */
        private HMM makeCopy(HMM hmm){
            ...
            HMM model = new HMM(npi,na,nb);
            return model;
        }
        /**
         * print the parameters learning curve.
         * Used for visualizing whether or not the algorithm converges
         * @param oldHmm the hmm before one iteration,
         * @param newHmm the hmm after one iteration
         */

        private void printLearningCurve(HMM oldHmm,HMM newHmm){
            double sumPi = 0.0;
            double sumA = 0.0;
            double sumB = 0.0;
        ...
        public int predict(O[] o, int[] st) {
            return predict(translate(o), st);
        }
    }
